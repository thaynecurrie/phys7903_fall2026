...
```
a,b=8.3,10
print("help, the value for a is {1:3s} and for b is {2:d}".format(a,b))
```
...
Using the Wikipedia page here -- <https://en.wikipedia.org/wiki/List_of_nearest_bright_stars> -- save the names of Sirius and Altair as two elements of a variable.  Do the same for their distances in light years.  

Then, create a file called ``nearestAstars.txt`` and populate it with the following two lines using the ``.write`` command, by converting from light-years to parsecs in a formatted print statement:
...
The numbers in the two above lines should not be hard-coded but created with formatted print statements using the ``{}`` modern formatting style.  Assume a conversion of 3.26 pc = 1 light-year.
...
* Write a revised version of the Python script from Question #3 that allows the user to EITHER run from command line OR to run by _import_ and then execute a function (from Python prompt, i.e. the thing likely starting with ">>>" on your computer). 

For the latter case, instead of just printing the answer, it should returns the value of the FWHM, where ``d_tel`` and ``lambda`` are free parameters that you can change when you call the function.  Demonstrate running this script and returning the result as a variable (i.e. >>> result = [the executed function]

### 5. Indentation/Running Code

Copy the following to a script named ``broken_script.py``. The intent of this script is to _import_ ``thecalc`` and run with different input values for ``thenumber``.  

```
import numpy as np

def thecalc(thenumber=5):
...
```
 ...
 Debug this script, fixing syntax errors.  Save a fixed version of this script in either a stand-alone file or in a Jupyter notebook cell.  Confirm that the fixed version of ``thecalc`` returns 0.5 if the input ``thenumber=1``, 50 if the input ``thenumber=3`` and 5.0 if ``thenumber=10.
 
### 6. Functions, If-Then Statements
...
### 8. Lists, Arrays, Loops and Type Conversions:
